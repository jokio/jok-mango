...
export default function updateFn<TDocument extends DocumentBase>(
  ...
  return async function update(
    ...
    options?: FindOneAndUpdateOption<TDocument> &
      ExtendOptionProps<Data<TDocument>>,
  ): Promise<TDocument> {
    const now = new Date()
    ...
    const version = options && options.skipVersionUpdate ? 1 : 0

    const updateQuery = options ? <any>options.updateQuery : null
...
type Data<TDocument extends DocumentBase> = Partial<
  Omit<
    TDocument,
